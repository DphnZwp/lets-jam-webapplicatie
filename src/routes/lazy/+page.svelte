...
// @ts-nocheck
  ...
  function toggleBold() {
      let input = document.getElementById("rich-text-input");
      let selectedText = input.value.substring(input.selectionStart, input.selectionEnd);

      let newText = input.value.substring(0, input.selectionStart) + 
                    "<b>" + selectedText + "</b>" +
  ...
  function countWords() {
      let input = document.getElementById("rich-text-input");
      wordCount = input.value.trim().split(" ").filter((item) => item).length;
  }
</script>

<main class="no-padding">
  <section class="lazy-introduction">
    <h1>Become a word wizard/witch</h1>
  </section>
  ...
  <section class="text-decoration">
    ...
    <button on:click={toggleUnderline}>
  ...
  <section class="lazy-writing">
    <textarea id="rich-text-input" bind:value={text} on:input={countWords}></textarea>
  </section>
</main>
